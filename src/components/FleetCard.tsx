    ...
    <article className={`fleet-card ${featured ? "fleet-card-featured" : ""}`}>
      ...
      <div className="fleet-content">
        ...
        <div className="fleet-card-actions">
          <a className="button fleet-card-action fleet-card-whatsapp" href={whatsappUrl(`Hello Get Car, please share availability and a final quote for ${car.name}.`)} target="_blank" rel="noreferrer" aria-label={`Book ${car.name} on WhatsApp`}>
            <WhatsAppIcon size={17} /> Book now on WhatsApp
          </a>
          <a className="button fleet-card-action fleet-card-call" href={`tel:${site.phoneHref}`} aria-label={`Call Get Car for ${car.name}`}>
